...
CURRICULUM_PATH = ROOT / "docs" / "curriculum-10a.md"
...
def extract_curriculum() -> dict:
    ...
    if not match:
        raise RuntimeError("未在 curriculum-10a.md 中找到机器可读 JSON 结构")
...
def topic_meta(subject_id: str, chapter_title: str, title: str) -> tuple[int, str, list[str]]:
    if subject_id == "chinese":
        if "写作" in title:
            return 3, "高考高频", ["#需大量练习"]
        if "整本书" in chapter_title or "家乡文化" in chapter_title or "词语积累" in chapter_title:
            return 2, "了解即可", ["#提高"]
        if "古诗词诵读" in chapter_title or chapter_title.startswith("第三单元"):
            return 3, "高考必考", ["#需背诵"]
        if "劝学" in title or "赤壁赋" in title or "登泰山记" in title or "师说" in title:
            return 4, "高考必考", ["#需背诵"]
        return 3, "高考高频", ["#基础"]
    if subject_id == "math":
        if any(key in title for key in ["基本不等式", "函数的基本性质", "指数函数", "对数函数", "三角函数的图象", "三角恒等变换", "y=Asin"]):
            return 4, "高考必考", ["#基础"]
        if "应用" in title:
            return 3, "高考高频", ["#需大量练习"]
        return 3, "高考必考", ["#基础"]
    ...
    if subject_id == "physics":
        if "实验" in title:
            return 3, "高考高频", ["#需大量练习"]
        if any(key in title for key in ["牛顿第二定律", "牛顿运动定律的应用", "共点力的平衡", "力的合成和分解"]):
            return 4, "高考必考", ["#基础"]
        return 3, "高考必考", ["#基础"]
    if subject_id == "chemistry":
        if "实验活动" in title:
            return 3, "高考高频", ["#需大量练习"]
        if any(key in title for key in ["离子反应", "氧化还原", "物质的量", "元素周期律", "化学键"]):
            return 4, "高考必考", ["#基础"]
        return 3, "高考必考", ["#基础"]
    if subject_id == "biology":
        if any(key in title for key in ["蛋白质", "细胞膜", "细胞呼吸", "光合作用", "细胞的增殖", "酶"]):
            return 4, "高考必考", ["#基础"]
        return 3, "高考高频", ["#基础"]
    if subject_id == "history":
        if any(key in title for key in ["秦统一", "变法运动", "隋唐制度", "两宋的政治", "两次鸦片战争", "挽救民族危亡"]):
            return 4, "高考必考", ["#需背诵"]
        return 3, "高考高频", ["#需背诵"]
    if subject_id == "geography":
        if any(key in title for key in ["大气受热过程", "水循环", "海水的运动", "圈层结构"]):
            return 4, "高考必考", ["#基础"]
        return 3, "高考高频", ["#基础"]
