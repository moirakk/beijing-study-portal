...
def extract_curriculum() -> dict:
    ...
    if not match:
        raise RuntimeError("未在 curriculum-9a.md 中找到机器可读 JSON 结构")
    return json.loads(match.group(1))


def topic_meta(subject_id: str, chapter_title: str, title: str) -> tuple[int, str, list[str]]:
    if subject_id == "chinese":
        if "写作" in title or "创作" in title or "朗诵" in title:
            return 2, "中考高频", ["#需大量练习"]
        if "整本书阅读" in title or "课外古诗词" in title:
            return 2, "中考高频", ["#需背诵"]
        if "综合性学习" in title:
            return 2, "中考高频", ["#提高"]
        return 3, "中考必考", ["#需背诵"] if chapter_title in {"第三单元", "第六单元"} else (2, "中考高频", ["#基础"])
    if subject_id == "math":
        if "课题学习" in title:
            return 2, "中考高频", ["#提高"]
        if any(key in title for key in ["二次函数", "一元二次方程", "圆", "概率"]):
            return 3, "中考必考", ["#基础"] if "实际问题" not in title else ["#需大量练习"]
        return 2, "中考高频", ["#基础"]
    ...
    if subject_id == "physics":
        if "欧姆定律" in chapter_title or "电流" in chapter_title or "电压" in chapter_title:
            return 3, "中考必考", ["#基础"]
        if "比热容" in title or "热机的效率" in title:
            return 3, "中考必考", ["#基础"]
        return 2, "中考高频", ["#基础"]
    if subject_id == "chemistry":
        if "化学方程式" in chapter_title or "氧气" in title or "二氧化碳" in title:
            return 3, "中考必考", ["#基础"]
        if "实验室" in title or "制取" in title:
            return 2, "中考高频", ["#实验"]
        return 2, "中考高频", ["#基础"]
    if subject_id == "history":
        if "活动课" in title:
            return 1, "了解即可", ["#提高"]
        if any(key in title for key in ["文艺复兴", "工业革命", "法国大革命", "美国的独立", "马克思主义"]):
            return 3, "中考必考", ["#需背诵"]
        return 2, "中考高频", ["#基础"]
    if subject_id == "politics":
        if any(key in title for key in ["改革开放", "共同富裕", "法治", "中国梦"]):
            return 3, "中考必考", ["#需背诵"]
        return 2, "中考高频", ["#基础"]
...
def update_subjects(curriculum: dict) -> dict:
    ...
    if "chemistry" not in subjects_by_id:
        insert_at = next((index for index, subject in enumerate(data["subjects"]) if subject["id"] == "biology"), len(data["subjects"]))
        data["subjects"].insert(
            insert_at,
            {
                ...
                "icon": "🧪",
                "grades": [
                    {"id": "9a", "title": "初三上", "textbook": "人教版", "chapters": []},
                    {"id": "9b", "title": "初三下", "textbook": "人教版", "chapters": []},
                    {"id": "10a", "title": "高一上", "textbook": "人教版", "chapters": []},
                    {"id": "10b", "title": "高一下", "textbook": "人教版", "chapters": []},
                    {"id": "11a", "title": "高二上", "textbook": "人教版", "chapters": []},
                    {"id": "11b", "title": "高二下", "textbook": "人教版", "chapters": []},
                    {"id": "12a", "title": "高三上", "textbook": "人教版", "chapters": []},
                    {"id": "12b", "title": "高三下", "textbook": "人教版", "chapters": []}
                ],
            },
        )
        subjects_by_id = {subject["id"]: subject for subject in data["subjects"]}
