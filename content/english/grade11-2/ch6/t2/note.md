...
```mermaid
flowchart TD
    A["避免重复的两种手段"] --> B["省略 ellipsis"]
    A --> C["替代 substitution"]
    B --> D["状语从句省略：When (you are) in danger, stay calm."]
    B --> E["不定式省略：I'd love to (go)."]
    C --> F["so / not 替代从句：I think so. / I hope not."]
    C --> G["one / that / do so 替代名词或动词"]
```
