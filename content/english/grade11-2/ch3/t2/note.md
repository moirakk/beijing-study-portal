...
```mermaid
flowchart TD
    A["分词作定语/状语"] --> B{"与逻辑主语的关系？"}
    B -->|"主动 / 进行"| C["现在分词 doing<br/>the rising sun"]
    B -->|"被动 / 完成"| D["过去分词 done<br/>a broken window"]
    C --> E["状语：Seeing the changes, we felt proud."]
    D --> F["状语：Seen from space, the city looks tiny."]
```
