...
```mermaid
flowchart TD
    A["非谓语空格：定语或状语"] --> B{"逻辑主语与该动词是什么关系？"}
    B -->|"主动（它发出动作）"| C["用 doing"]
    B -->|"被动（它承受动作）"| D["用 done"]
    D --> E{"是不及物动词表完成？（fallen/risen）"}
    E -->|是| F["done 只表完成不表被动"]
    E -->|否| G["done 表被动兼完成"]
```
...
[[词汇与课文精读（Understanding ideas）]] ｜ [[读写与表达（Developing ideas）]] ｜ 对比 [[语法与语言运用（Using language）]]（-ing 形式）
