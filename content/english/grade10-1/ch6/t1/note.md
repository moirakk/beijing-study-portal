...
```mermaid
flowchart TD
    A["At one with nature（人与自然）"] --> B["地形与景观"]
    A --> C["农耕与建造"]
    A --> D["理念与价值"]
    B --> B1["terrace, slope, steep, landscape"]
    B --> B2["breathtaking view"]
    C --> C1["crop, harvest, irrigate, channel"]
    C --> C2["construct, by hand"]
    D --> D1["harmony, wisdom, ancestor"]
    D --> D2["sustainable, be home to"]
```
...
[[语法与语言运用（Using language）]] ｜ [[词汇与课文精读（Understanding ideas）]]
