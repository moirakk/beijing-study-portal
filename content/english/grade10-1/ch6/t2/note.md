...
```mermaid
flowchart TD
    A["动词填空两步法"] --> B{"第一步：主语是动作的执行者还是承受者？"}
    B -->|执行者| C["主动语态"]
    B -->|承受者| D["被动语态 be + done"]
    D --> E{"第二步：判断时态"}
    E -->|"现在的习惯/事实（every year, usually）"| F["is/are + done"]
    E -->|"过去的动作（in 1900, years ago）"| G["was/were + done"]
    D --> H["注意：不及物动词（happen, take place, date back to）无被动"]
```
