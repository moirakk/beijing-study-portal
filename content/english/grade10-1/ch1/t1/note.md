...
```mermaid
flowchart TD
    A["A new start（新起点）"] --> B["情绪词汇"]
    A --> C["校园词汇"]
    A --> D["行为词汇"]
    B --> B1["nervous, anxious, embarrassed"]
    B --> B2["curious, impressive"]
    C --> C1["campus, impression, talent"]
    D --> D1["recommend, organise, refer to"]
    D --> D2["look forward to, make an impression"]
```
