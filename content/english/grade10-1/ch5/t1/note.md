...
```mermaid
flowchart TD
    A["Into the wild（动物迁徙）"] --> B["迁徙过程"]
    A --> C["生存挑战"]
    A --> D["科学之谜"]
    B --> B1["migrate, journey, destination"]
    B --> B2["set off, up to 4,000 km"]
    C --> C1["survive, shelter, temperature"]
    C --> C2["threat, habitat loss"]
    D --> D1["instinct, remain a mystery"]
    D --> D2["four generations（几代接力）"]
```
