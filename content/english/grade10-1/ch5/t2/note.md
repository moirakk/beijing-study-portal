...
```mermaid
flowchart TD
    A["动词填空：完成时还是过去时？"] --> B{"有明确过去时间？（yesterday/ago/last.../just now）"}
    B -->|是| C["一般过去时 did"]
    B -->|否| D{"有 since/for/so far/already/yet/ever？"}
    D -->|是| E["现在完成时 have/has done"]
    D -->|否| F{"强调对现在的影响或到现在为止的经历？"}
    F -->|是| E
    F -->|否| G["结合上下文时态基调判断"]
    E --> H["注意：延续用法须用延续性动词（be, keep, have），非延续动词（buy, die, join）不与 for/since 直接连用"]
```
