...
```mermaid
flowchart TD
    A["主谓一致判断"] --> B{"主语后有 with/as well as/together with 插入语？"}
    B -->|是| C["谓语随前面的中心主语"]
    B -->|否| D{"是 or / either...or / neither...nor 连接？"}
    D -->|是| E["就近一致：谓语随最近的主语"]
    D -->|否| F{"是 each/every/nobody 等不定代词？"}
    F -->|是| G["谓语用单数"]
    F -->|否| H["按主语单复数常规判断（注意 the majority of, a number of 用复数）"]
```
