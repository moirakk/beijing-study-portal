...
| 概念 | 定义 |
|---|---|
| 化学式 | 用元素符号和数字表示物质组成的式子 |
| 化合价 | 原子形成化合物时表现的性质，有正负价 |
| 相对分子质量 | 化学式中各原子相对原子质量的总和 |

## 化学式的意义（以H₂O为例）

- 宏观：表示水；表示水由氢、氧元素组成。微观：表示1个水分子；1个水分子由2个氢原子和1个氧原子构成。
- 数字含义：前面数字为粒子个数；右下角为分子中原子个数；右上角为离子电荷；正上方为化合价。
...
口诀：一价钾钠氯氢银，二价氧钙钡镁锌；三铝四硅五价磷，二三铁、二四碳；二四六硫都齐全，铜汞二价最常见。
...
- 化合物中正负化合价代数和为零；单质中化合价为0；氧通常-2价、氢通常+1价，金属显正价。
- 书写"正前负后、约简交叉"：化合价数值约成最简比后交叉写在右下角。例：氧化铝Al(+3)、O(-2)→Al₂O₃。
...
$$
M(CO_2) = 12 + 16 \times 2 = 44,\quad m_C : m_O = 12 : 32 = 3 : 8,\quad C\% = \frac{12}{44} \times 100\% \approx 27.3\%
$$
...
1. 化合价口诀与常见原子团符号、价数熟背。
2. 化合物中正负化合价代数和为零；单质中为0。
3. 化学式书写"正前负后、交叉约简"。
4. 相对分子质量注意乘原子个数；元素质量比也要乘原子个数。

## 自测题

1. 标出KMnO₄中Mn的化合价，并写出硫酸铝的化学式。
2. 计算NH₄NO₃的相对分子质量及氮元素的质量分数（保留一位小数）。
